...
pub async fn login_user(
    ...
    // let status = hash_response.status();
    ...
    let old_master_password_hash = hash_response["master_password"].as_str().unwrap_or_default().to_string();
    println!("passwort: {}", old_master_password_hash);
    return Ok(hash_response);
